...
def get_users(data_bag, search_group)
  users = []
  if search_missing?
    Chef::Log.warn('This recipe uses search. Chef Solo does not support search unless you install the chef-solo-search cookbook.')
  else
    db = data_bag(data_bag)
    db.each do |item_name|
      item = Chef::DataBagItem.load(data_bag, item_name).to_hash
      if item['type'].respond_to?('has_key?') && item['type'].has_key?('encrypted_data')
        item = Chef::EncryptedDataBagItem.load(data_bag], item_name).to_hash
      end
      next if item.has_key?('environments') and ! item['environments'].include?(node.chef_environment)
      next unless item['groups'].include?(search_group)
      users << item
    end
  end
  users
...
action :remove do
  get_users(new_resource.data_bag, new_resource.search_group).each do |rm_user|
    next unless rm_user['action'] == 'remove'
...
action :create do
  ...
  get_users(new_resource.data_bag, new_resource.search_group).each do |u|
    next if u['action'] == 'remove'
